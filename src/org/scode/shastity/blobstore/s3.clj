...
(deftype S3Store [s3-client bucket-name path]
  ...
  (has-blob [store name] (not (nil? (.getObjectMetadata s3-client bucket-name (str path name)))))
  (get-blob [store name] (let [s3-object (.getObject s3-client bucket-name (str path name))
